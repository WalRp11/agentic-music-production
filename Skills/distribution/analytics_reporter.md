...
You have two options — offer both to the human:

### Option A: YouTube Studio CSV Export (recommended for solo operator — faster)

```
In YouTube Studio:
  1. Go to Analytics → Content → [video]
  2. Set date range: last [48h / 7d / 30d]
  3. Click the download icon (top right) → Export to CSV
  4. Save as: Distribution/youtube_analytics_[YYYY-MM-DD].csv
  5. Tell me: "CSV is ready"

I will parse the CSV and extract all metrics automatically.
```

When the CSV is provided, extract:
- `views`, `watchTimeHours`, `averageViewDuration`, `averageViewPercentage`
- `impressionClickThroughRate`, `impressions`, `uniqueViewers`
- `subscribersGained`, `topTrafficSource`

Parse with Python if available:
```python
import csv
with open('Distribution/youtube_analytics_[date].csv') as f:
    reader = csv.DictReader(f)
    for row in reader:
        print(row)  # extract relevant fields
```

### Option B: Manual Copy-Paste

If CSV export is not available, ask the human to provide:
